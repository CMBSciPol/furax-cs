...
# Mapping from bin_config param name → (patch_key, param_key)
...
def bin_patches(
    ...
    """Quantize patch assignments by binning recovered spectral-parameter maps.

    For each parameter in *bin_config* the recovered parameter map is divided
    into equal-width bins and the patch assignment array is replaced with the
    bin index.  The parameter arrays are also replaced with bin-center values
    so that downstream code (e.g. ``params_to_maps``) can index them with the
    new bin indices.

    This function operates on *indexed* run data (after ``index_run_data``),
    i.e. arrays have shapes ``(n_real, ...)`` for parameters and ``(npix,)``
    for patches.

    Parameters
    ----------
    run_data : dict[str, np.ndarray]
        Single-grid-point slice of a ``results.npz`` dictionary.
        Modified **in-place** and returned.
    bin_config : dict[str, int]
        Mapping from parameter name (``"beta_dust"``, ``"temp_dust"``,
        ``"beta_pl"``) to the desired number of bins.
    noise_selection : str
        Strategy to pick the reference noise realization whose recovered
        parameter values define the bin edges: ``"min-value"`` (default),
        ``"min-nll"``, or an integer index string.

    Returns
    -------
    dict[str, np.ndarray]
        The (mutated) *run_data* dictionary with updated patch and param arrays.
    """
    # Select reference realization — run_data["value"] has shape (n_real,)
    if noise_selection == "min-nll":
        ref_idx = int(np.argmin(run_data["NLL"]))
    elif noise_selection == "min-value":
        ref_idx = int(np.argmin(run_data["value"]))
    else:
        ref_idx = int(noise_selection)

    for param_name, nbins in bin_config.items():
        patch_key, value_key = _BIN_PARAM_KEYS[param_name]

        param_values = run_data[value_key][ref_idx]  # (n_clusters,)
        patches = run_data[patch_key]  # (npix,)

        param_map = param_values[patches]  # (npix,)
        valid = param_map > -1e30  # exclude hp.UNSEEN sentinel

        vmin = param_map[valid].min()
        vmax = param_map[valid].max()
        bin_edges = np.linspace(vmin, vmax + 1e-10, nbins + 1)
        binned = np.digitize(param_map, bin_edges)  # values in [1, nbins]
        binned[~valid] = 0

        run_data[patch_key] = binned

        # Build bin-center param array compatible with binned patch indices.
        # Index 0 = invalid (hp.UNSEEN region), indices 1..nbins = bin centers.
        n_bins_used = int(binned.max())
        bin_centers = np.zeros(n_bins_used + 1)
        bin_centers[1:] = ((bin_edges[:-1] + bin_edges[1:]) / 2)[:n_bins_used]
        n_real = run_data[value_key].shape[0]
        run_data[value_key] = np.tile(bin_centers, (n_real, 1))
